...
RedisProxy.prototype.readyup = function(active){
  ...
  this.connections = new Pool({create: function(cb){
    try {
  ...
  }, size: this.options.pool_size, close: function close(conn){
    conn.quit();
  }});
  active.slavenone();
...
RedisProxy.prototype.sendCommand = function(command, callback) {
  return this.connections.random().sendRaw(command, callback);
};
