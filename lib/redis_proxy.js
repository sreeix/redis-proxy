  ...
  this.allServers = _.map(o.servers, function(server){
    return new Server(server).on('up', function() {
      ...
      if(_.isNull(self._active) || _.isUndefined(self._active)){
        self._active = this;
        console.log("setting up the active "+ self._active.options.host + ":" + self._active.options.port);
        self.readyup(this);
      } else {
        console.log("Slaving "+ this.options.host+":"+this.options.port + "to" + self._active.options.host+ ""+ self._active.options.port);
...
RedisProxy.prototype.readyup = function(active){
  console.log("Creating the pool for active server"+ active.options.port);
...
RedisProxy.prototype.watch = function(){
  ...
  setInterval(function(){
    ...
    _.each(self.allServers, function(server){
      console.log("Pinging "+ server.options.host +":"+ server.options.port);
      server.ping();
    });
